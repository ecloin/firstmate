...
#
# Every value that grows with fleet data reaches jq through fm_jq
# (bin/fm-jq-lib.sh), never through --arg/--argjson: argv caps one argument at
# 128 KiB, so a grown backlog on argv breaks the whole snapshot.
set -u
...
crew_state_json() {  # <id>
  ...
  fm_jq raw "$(fm_jq_text "$raw")" detail "$(fm_jq_text "$detail")" -- \
    -n --arg state "$state" --arg source "$source" \
...
status_event_json() {  # <status-log>
  ...
  # shellcheck disable=SC2016 # jq owns every $ expression in this literal program.
  fm_jq raw "$(fm_jq_text "$raw")" verb "$(fm_jq_text "$verb")" note "$(fm_jq_text "$note")" -- \
    -n \
    --arg path "$log" \
...
task_json_lines() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    fm_jq \
      ...
      last_event_raw "$(fm_jq_text "$last_event_raw")" \
      pr "$(fm_jq_text "$pr")" \
      -- \
      -n \
      --arg id "$id" \
...
parent_evidence_reconciliation_json() {  # <summary-json> <activities-json> <decisions-json>
  # shellcheck disable=SC2016 # jq owns every $ expression in this literal program.
  fm_jq summary "$1" activities "$2" decisions "$3" -- -n '
...
secondmate_current_json() {  # <parent-tasks-json>
  ...
  # shellcheck disable=SC2016 # jq owns every $ expression in this literal program.
  union=$(fm_jq registry "$registry" tasks "$tasks" -- -n '
  ...
  while IFS= read -r row; do
    ...
    if [ -z "$reason" ]; then
      ...
      # shellcheck disable=SC2016 # jq owns every $ expression in this literal program.
      terminal_contradiction=$(fm_jq reconciliation "$reconciliation" note "$(fm_jq_text "$event_note")" -- -n -r '
        $reconciliation | any(.activities[]; .verdict == "contradicts" and .summary == $note)')
      ...
      # shellcheck disable=SC2016 # jq owns every $ expression in this literal program.
      record=$(fm_jq \
        summary "$summary" decisions "$decisions" activities "$activities" \
        activity_scan "$activity_scan" reconciliation "$reconciliation" terminal "$terminal" \
        current_reason "$(fm_jq_text "$current_reason")" \
        event_raw "$(fm_jq_text "$event_raw")" event_note "$(fm_jq_text "$event_note")" -- \
        -n \
      ...
      # shellcheck disable=SC2016 # jq owns every $ expression in this literal program.
      record=$(fm_jq \
        activities "$activities" activity_scan "$activity_scan" decisions "$decisions" terminal "$terminal" \
        reason "$(fm_jq_text "$reason")" \
        event_raw "$(fm_jq_text "$event_raw")" event_note "$(fm_jq_text "$event_note")" -- \
        -n \
    ...
    # shellcheck disable=SC2016 # jq owns every $ expression in this literal program.
    records=$(fm_jq records "$records" record "$record" -- -n '$records + [$record]')
  done <<EOF
$rows
EOF
  # shellcheck disable=SC2016 # jq owns every $ expression in this literal program.
  fm_jq registry "$(fm_jq union "$union" -- -n '$union.registry')" records "$records" -- \
    -n \
