...
# A ship task's review marker - the `<task-id>-reviewed` tag bin/fm-brief.sh's
# local-only contract has the worker pin in the shared project clone - is removed
# with the rest of the task's durable records, after every landed-work refusal has
# passed. Only that exact tag is deleted; a task that never created one is a silent
# no-op, and no other tag in the clone is touched.
...
# A local-only ship task keeps a `<task-id>-reviewed` git tag in the shared
# project clone as the reviewer's pinned since-last-review base (generated by
# bin/fm-brief.sh's review-marker contract). Once the work has landed nothing
# consumes it, so retire exactly that one ref and leave every other tag - the
# project's releases and the captain's own markers - untouched. A missing tag is
# the normal case for other delivery modes and is not an error. Called only
# after every landed-work refusal has passed, alongside the other durable
# per-task record removals, so an aborted teardown leaves the marker intact for
# a rerun.
remove_review_marker_tag() {  # <project> <task-id>
  local proj=$1 id=$2 tag
  tag="$id-reviewed"
  [ -n "$proj" ] && [ -d "$proj" ] || return 0
  git -C "$proj" rev-parse --verify --quiet "refs/tags/$tag" >/dev/null 2>&1 || return 0
  git -C "$proj" tag -d "$tag" >/dev/null 2>&1 \
    || echo "warning: could not remove review marker tag $tag in $proj" >&2
  return 0
}
...
# Ship tasks only: a scout worktree is scratch and a secondmate home has no task
# branch, so neither ever carries a review marker.
[ "$KIND" != ship ] || remove_review_marker_tag "$PROJ" "$ID"
