...
| `fm-fleet-view.sh`       | Render the fleet snapshot for a person: the narrow needs-you-first captain sidebar by default, `--wide` for Markdown tables ([`docs/fleet-view.md`](fleet-view.md)) |
...
| `fm-jq-lib.sh`           | Single owner of handing jq a value whose size is unbounded, so no fleet aggregate rides on argv past the kernel's single-argument cap |
